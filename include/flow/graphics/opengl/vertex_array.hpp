...
namespace gl {
    ...
    public:
        ...
        // clang-format off
        using deleter_type = decltype([](id_type id) { glDeleteVertexArrays(1, &id); });
        // clang-format on
    ...
    public:
        ...
        void set_vertex_buffer(index_type binding_index, id_type buffer_id, size_type stride, offset_type offset = 0) const noexcept
        {
        ...
        void set_vertex_buffer(index_type binding_index, const buffer<>& buffer, size_type stride, offset_type offset = 0) const noexcept
        {
            set_vertex_buffer(binding_index, buffer.id(), stride, offset);
        }
        ...
        void set_element_buffer(const buffer<>& buffer) const noexcept
        {
        ...
        void set_attribute_config(index_type attribute_index, const attribute_format& format, bool normalize) const noexcept
        {
        ...
        void set_attribute(index_type binding_index, index_type attribute_index, const attribute_format& format, bool normalize, bool enabled = true) const noexcept
        {
            set_attribute_binding(binding_index, attribute_index);
        ...
        void set_attribute_bindings(index_type binding_index, std::initializer_list<index_type> attribute_indices) const noexcept
        {
            for (auto index : attribute_indices)
            {
                set_attribute_binding(binding_index, index);
